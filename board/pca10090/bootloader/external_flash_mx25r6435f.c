...
#include "external_flash_mx25r6435f.h"
#include "nrf_gpio.h"
#include "nrf_spim.h"

#define MIN(x,y)    (x < y? x : y)

// https://www.macronix.com/Lists/Datasheet/Attachments/8746/MX25R6435F,%20Wide%20Range,%2064Mb,%20v1.5.pdf
...
static bool ext_flash_write_enable()
{
    ...
    cmd[0] = MX25R6435F_CMD_WRITE_ENABLE;
    ...
    cmd[0] = MX25R6435F_CMD_READ_STATUS;
    ext_flash_select(true);
    ext_flash_spi_xfer(cmd, sizeof(cmd), reply, sizeof(reply), false);
    ext_flash_select(false);
    return (reply[1] & MX25R6435F_STATUS_WEL) == MX25R6435F_STATUS_WEL;
}
...
static bool ext_flash_set_high_performance_mode()
{
    ...
    cmd[0] = MX25R6435F_CMD_WRITE_STATUS;
...
extFlash_res_e externalFlash_init(void)
{
     ...
     * with terminology used in MX25R6435F manual, where it is stated that the external flash
    ...
    {
        cmd[0] = MX25R6435F_CMD_READ_IDENTIFICATION;

        ext_flash_select(true);
        ext_flash_spi_xfer(cmd, sizeof(cmd), reply, sizeof(reply), false);
        ext_flash_select(false);

        if ((reply[1] == MX25R6435F_ID_1) &&
            (reply[2] == MX25R6435F_ID_2) &&
            (reply[3] == MX25R6435F_ID_3))
        {
...
extFlash_res_e externalFlash_startRead(void * to, const void * from, size_t amount)
{
    ...
    cmd[0] = MX25R6435F_CMD_READ_ARRAY;
    ...
    while (amount > 0) {
        uint32_t bytesToRead = MIN(amount, MX25R6435F_MAX_TRANSFER_SIZE);
...
extFlash_res_e externalFlash_startWrite(void * to, const void * from, size_t amount)
{
    ...
    cmd[0] = MX25R6435F_CMD_PROGRAM_PAGE;
    set_address(&cmd[1], address);
    ...
    while (amount > 0)
    {
        uint32_t bytesToWrite = MIN(amount, MX25R6435F_MAX_TRANSFER_SIZE);
...
bool externalFlash_isBusy(void)
{
    ...
    ext_flash_spi_xfer(cmd, sizeof(cmd), reply, sizeof(reply), false);
    ext_flash_select(false);
    return (reply[1] & MX25R6435F_STATUS_WIP) == MX25R6435F_STATUS_WIP;
}
...
extFlash_res_e externalFlash_startErase(size_t * sector_base, size_t * number_of_sector)
{
    ...
    cmd[0] = MX25R6435F_CMD_BLOCK_ERASE_32K;
    set_address(&cmd[1], address);
