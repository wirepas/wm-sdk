...
static void dispatch_tlv(uint16_t type, uint8_t len, uint8_t * val)
{
    ...
    for (uint8_t i = 0; i < SHARED_APP_CONFIG_MAX_FILTER; i++)
    {
        ...
        {
            LOG(LVL_DEBUG, "type %d vs %d", type, filter_entry.type);
            if ((filter_entry.type == SHARED_APP_CONFIG_ALL_TYPE_FILTER)
                 || ((uint16_t) filter_entry.type == type))
            {
                filter_entry.cb(type, len, val);
            }
        }
    }
                              ...
                              uint16_t interval)
{
    ...
    {
        LOG(LVL_WARNING,
            "App config with wrong header: 0x%x",
            header->version);
    ...
    while (entry_number--)
    {
        ...
        dispatch_tlv(item.type, item.length, item.value);
    }
}
                            ...
                            uint16_t * filter_id)
{
    ...
    /* Check filter parameter. Either a uint16_t or SHARED_APP_CONFIG_ALL_TYPE_FILTER */
    if ((((filter->type < 0) || (filter->type > (UINT16_MAX)))) &&
        (filter->type != SHARED_APP_CONFIG_ALL_TYPE_FILTER))
    {
        LOG(LVL_ERROR, "Invalid type %d", filter->type);
        return SHARED_APP_CONFIG_RES_INVALID_PARAM;
    }
...
shared_app_config_res_e Shared_Appconfig_removeFilter(uint16_t filter_id)
{
    ...
    LOG(LVL_DEBUG,
        "Remove app conf filter (id: %d)",
        filter_id);

    shared_app_config_res_e res = SHARED_APP_CONFIG_RES_OK;
    Sys_enterCriticalSection();
    if (m_filter[filter_id].filter.cb != NULL)
    {
