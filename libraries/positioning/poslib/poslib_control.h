 ...
 *          used PosLib configuration. When oneshot was requested restores
 *          PosLib state to state before oneshot was called and restarts
 *          PosLib if needed.
 */
 ...
 * @return  poslib_ret_e
 *          POS_RET_OK if no errors
 *          POS_RET_INVALID_STATE if init is not done
 */
...
/**
 * @brief   Stops task run_poscontrol_state_machine activity and removes
 *          shared libraries.
 */
