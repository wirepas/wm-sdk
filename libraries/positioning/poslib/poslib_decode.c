    ...
    poslib_settings_t * settings)
{
    ...
    settings->update_period_bleon_offline_s = command->waittime;

    switch(waittime)
    {
        case POSLIB_DECODE_BEACON_OFF:
        {
            PosLibBle_setMode(POSLIB_BLE_BEACON_OFF);
            settings->poslib_ble_settings.ble_mode = POSLIB_BLE_BEACON_OFF;
        }
        break;
        case POSLIB_DECODE_BEACON_ON:
        {
            PosLibBle_setMode(POSLIB_BLE_BEACON_ON);
            settings->poslib_ble_settings.ble_mode = POSLIB_BLE_BEACON_ON;
        }
        break;
        default:
        {
            PosLibBle_setMode(POSLIB_BLE_BEACON_ON_WHEN_OFFLINE);
            settings->poslib_ble_settings.ble_mode =
                POSLIB_BLE_BEACON_ON_WHEN_OFFLINE;
        }
        break;
    }
