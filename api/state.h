...
/**
 * @brief   Structure for adjusting hops
 */
typedef struct
{
    /** Target address of the packet (multicast group, with @ref
         APP_ADDR_MULTICAST bitmask set) */
    app_addr_t  address;
...
/**
 * @brief   Function type for a adjusting hops left
 * @param   info
...
/**
 * @brief   Set a callback to be called when rerouting packet and querying
 *          if 'hops left' should be modified (i.e. limiting the propagation of
 *          packet further)
 * @param   cb
