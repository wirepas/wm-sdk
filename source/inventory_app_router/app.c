...
typedef struct
{
    uint8_t ack_data[DIRADV_MAX_ACK_LEN];
...
bool appAckDataCb(const ack_gen_input_t * in, ack_gen_output_t * out)
{
    (void) in;
...
static void appCfgDataCb(const uint8_t * bytes,
                        uint8_t seq,
                        uint16_t interval)
{
    uint8_t max_size = lib_data->getAppConfigNumBytes();
    app_config_data_t app_data;
    uint8_t magic = *bytes;

    if (sizeof(app_config_data_t) <= max_size &&
        m_settings.app_seq != seq &&
        magic == 0xAD)
    {
...
uint8_t  pack_msg( adv_data_type2_t * payload, uint8_t payload_type, uint32_t msg_count)
{
...
static uint32_t send_data(void)
{
    ...
    m_msg_count = (m_msg_count == 65534) ? 1 : (m_msg_count + 1);
...
static void set_callbacks(void)
{
    ...
    lib_data->setNewAppConfigCb(appCfgDataCb);
