...
#include "mcu.h"
#include "nfc_hw.h"
#include "t2_emulation.h" /* used for t2emu_cmd_e */
...
#define NFC_DEFAULTSTATESLEEP     (*(uint32_t volatile *)(0x40005420))
...
/* Wiating for field on confirmation */
...
void __attribute__((__interrupt__)) IRQ_handler(void)
{
    ...
    if ((NRF_NFCT->EVENTS_RXFRAMEEND) && (NRF_NFCT->INTEN & NFCT_INTEN_RXFRAMEEND_Msk))
    {
        ...
        {
            /* Frame is not good, wait for next frame reception */
...
nfc_hw_res_e nfc_hw_init(hw_nfc_callback_f callback, uint8_t *buffer, uint16_t size)
{
