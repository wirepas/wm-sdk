...
static void enable_clock(bool enable)
{
    if (enable)
    {
        // Turn on clock to the I2C peripheral
        CMU->HFPERCLKEN0 |= I2C_HFPERCLKEN0_BIT;
    }
    else
    {
        // Turn off clock to the I2C peripheral
        CMU->HFPERCLKEN0 &= ~I2C_HFPERCLKEN0_BIT;
    }
}
...
static bool set_frequency(uint32_t f_scl)
{
    ...
    uint32_t f_hfperclk = CMU_ClockFreqGet(cmuClock_HFPER);

    // Calculate required divisor
    //
    // div = (f_hfperclk / f_scl - 8) / (n_low + n_high) - 1
    //
    // , where
    //      f_hfperclk: HFPERCLK frequency, in Hz
...
static void i2c_state_machine(i2c_sm_command_e command)
{
    ...
    uint8_t dev_state = I2C_DEV->STATE & (_I2C_STATE_STATE_MASK |
    ...
    else if ((command == I2C_SM_CMD_NEXT) && (m_state == I2C_SM_ST_STOP))
    {
        ...
        if (dev_state == I2C_STATE_STATE_IDLE)
        {
            // Call callback
            call_cb = true;
        }
